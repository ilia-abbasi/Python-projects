...
def handle_mode(mode : str) -> str:
    ...
    if mode != "Y" and mode != "N":
        ...
        raise Exception("Invalid mode: only Y or N is accepted.")
...
def handle_action(action : str) -> str:
    ...
    if action == "":
        ...
        raise Exception("Invalid action: no action.")

    action = action.upper()

    action = action[0]

    if action != "L" and action != "U":

        raise Exception("Invalid action: only L or U is accepted.")
